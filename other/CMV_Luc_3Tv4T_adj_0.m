...
folList = {'4T-APS CMV-Luc Raw','3T-APS CMV-Luc Raw'};
...
for fileI = 1:nTrial
    ...
    %     %%Prelim Figure 6 - check difference between all normal and aMatrix
    %     figure
    %     subplot(2,1,1)
    %     imagesc(all_normal_matrix(:,:,1));
    %     colorbar
    %     subplot(2,1,2)
    %     imagesc(aMatrix(:,:,1));
    %     colorbar
    %     title([curFol,' before and after trimming']);
...
%     %%Figure to check matrix processing
%     switch fileI
%         case {2}
%             frame_select1 = 112;
%         case {1}
...
%     caxis ([minVal maxVal])
%     colorbar
%     title(['Matrix before trimming']);
%     subplot(5,2,[3,4])
%     imagesc(aMatrix(:,:,frame_select1));
%     caxis ([minVal maxVal])
%     colorbar
%     title(['Matrix after trimming']);
%     subplot(5,2,5)
...
%     title(['Normalized Matrix']);
    ...
    for ijk = 1:size(F0_id,2)
        xline(F0_id(ijk),'LineWidth',1.5); %%Label raw Frame0s
    end
...
%%%% NOTES
% wo_filter matrix and w_filter matrix were negatived,
% assignment of w/ and w/o filter is different for 4T and 3T
% Promotorless somehow has a signal
