...
for ij = 1:2
    ...
    set(p2, 'Position', [0.045+0.5*(1-mod(ij,2)) 0.6-0.44 0.42 0.745])
end
...
cell{1} = olddev.diff_Matrix(:,:,oldF0+1:end);
cell{2} = newdev.diff_Matrix(:,:, newF0+1:end);
...
for ij = 1:2
    ...
    min_bin = min( min(withcell(~isinf(withcell))), min(nocell(~isinf(nocell))) );
    max_bin = max( max(withcell(~isinf(withcell))), max(nocell(~isinf(nocell))) );
    ...
    %    max(nocell(:)));
    cell_hist = histogram(withcell, bin, 'EdgeColor', 'none',...
        'Normalization','probability');
    cell_pdf = cell_hist.Values;
    hold on
    null_hist = histogram(nocell, bin, 'EdgeColor', 'none',...
        'Normalization','probability');
    ...
    set(p3, 'Position', [0.054+0.5*(1-mod(ij,2)) 0.6-0.44 0.42 0.745])
...
f3 = figure( 'Units', 'normalized', 'Position', [0.1 0.25 1 0.45] );
...
%     'With Cells\newline(4T-APS CMV-Luc)'};
% 
% f4 = figure( 'Units', 'normalized', 'Position', [0.1 0.25 0.8 0.47] );
...
% title('Pairwise Hellinger Distances (Difference)');
% 
